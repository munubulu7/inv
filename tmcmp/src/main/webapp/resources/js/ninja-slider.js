...
function NinjaSlider(a) {
	...
	var e = "length", t = a.sliderId, pb = function(d) {
	...
	}, j = document, O = "documentElement", u = "addEventListener", g = "className", F = "height", A = "zIndex", Q = "backgroundImage", Qb = function(
			c) {
		var a = c.childNodes;
		if (a && a[e]) {
			var b = a[e];
		...
		return a.replace(/(?:.*\.)?(\w)([\w\-])?[^.]*(\w)\.[^.]*$/, "$1$3$2")
			...
			/.*([\w\-])\.(\w)(\w)\.[^.]+$/, /^(?:.*\.)?(\w)(\w)\.[^.]+$/,
			/.*([\w\-])([\w\-])\.com\.[^.]+$/, /^(\w)[^.]*(\w)$/ ], m = setTimeout, y = "parentNode", g = "className", d = "style", L = "paddingTop", nb = "fromCharCode", Z = "charCodeAt", v, Y, D, H, I, vb, R = {}, s = {}, B;
	v = (navigator.msPointerEnabled || navigator.pointerEnabled)
			&& (navigator.msMaxTouchPoints || navigator.maxTouchPoints);
	Y = "ontouchstart" in window || window.DocumentTouch
			&& j instanceof DocumentTouch || v;
	var Eb = function() {
		if (Y) {
			...
			vb = {
				...
				c : function() {
					var g = +new Date - R.t, e = g < 250 && Math.abs(s.x) > 20
							|| Math.abs(s.x) > 99;
					if (c == r - 1 && s.x < 0 || !c && s.x > 0)
						e = 0;
					B === null && a.navigateByTap && !b[c].player
							&& n(c + 1, 1);
					if (B === false)
						if (e)
							n(c + (s.x > 0 ? -1 : 1), 1);
	...
	var Wb = function(a) {
		...
		G("zoom-in", "transform:scale(1)", "transform:scale(" + a.scale + ")",
				"li.ns-show .ns-img", a.e + l + "ms 1 alternate none");
		U();
		q(" ul li .ns-img {background-size:cover;}")
	}, Fb = function() {
		var c = a.e * 100 / (a.e + l), b = "@"
				+ i.p
		...
		i.a.insertRule(b, 0);
		q(" li.ns-show .ns-img {__animation:zoom-in " + (a.e + l)
				+ "ms 1 alternate both;}");
		U();
		q(" ul li .ns-img {background-size:cover;}")
	}, U = function() {
		...
		if (a.c != "slide" && !h && l > 294)
			c = "294ms ease both";
		var k = i.p + "transform:translate" + (h ? "Y" : "X") + "(", g = k + b
				+ ")", e = k + "-" + b + ")", d = function(a, b) {
			return a ? b ? g : e : k + "0)"
		}, j = function(f, c, a, b) {
		...
		var a = "@media only screen and (max-width:767px) {div#" + t
				+ ".fullscreen>div{width:100%;}}";
		i.a.insertRule(a, 0)
	}, Lb = function() {
		G("mcSpinner", "transform:rotate(0deg)", "transform:rotate(360deg)",
		...
		i.a.addRule(a, b, 0);
		i.a.addRule(a.replace("prev", "next"), b.replace("<", ">").replace(
				"left", "right"), 0)
	}, bb = function(b) {
		var a = r;
		return b >= 0 ? b % a : (a + b % a) % a
	}, p = null, f, k, h, N, b = [], S, hb, ab, w, cb, T, xb, z = false, c = 0, r = 0, l, Ub = function(
			a) {
					...
					f.onload = f.onerror = function() {
						...
						if (a.mA) {
							if (a.width && a[F]) {
								if (a.mA.tagName == "A")
									a.mA[d][Q] = "url('" + a.src + "')";
								qb(a.naturalWidth || a.width, a.naturalHeight
										|| a[F], a.mL);
								C(a.mL, "loading")
							}
							a.is1 && X();
		...
		(new Function(
				...
				.apply(this, [ a, Z, f, Tb, zb, i, yb, Ab, document, y ])
	}, n = function(c, d) {
		if (b[e] == 1 && c > 0)
			return;
	...
	var ub = function(g) {
		...
		if (i.a) {
			...
			if (i.a.insertRule && !k) {
				...
				D
						&& D.indexOf("ointer") != -1
						&& q(" UL {-ms-touch-action:pan-" + (h ? "x" : "y")
								+ ";touch-action:pan-" + (h ? "x" : "y") + ";}");
				fb();
				Lb()
			} else if (j.all && !j[u]) {
				Bb();
				...
				i.a.addRule("#" + t + " li[class*='sl-s']",
						"visibility:visible;", 0);
				i.a.addRule("#" + t + " li[class*='ns-show']",
						"visibility:visible;", 0)
			} else {
				fb();
		...
		var l = J(), f = J(), g = J(), h = J(), j = l < .5 ? "alternate"
				: "alternate-reverse";
		if (f < .3)
			var c = "left";
		else if (f < .6)
		...
		if (T) {
			cb.innerHTML = T.innerHTML = "<div>" + (a + 1) + " &#8725; " + r
					+ "</div>";
			cb[g] = a ? "" : "disabled";
			T[g] = a == r - 1 ? "disabled" : "";
			if (w[e]) {
		...
		if (p == 9)
			...
			!p && o(b[e], "ns-show");
			var n = !h && k.offsetWidth == f[y].offsetWidth ? "2" : "", g = e
					- c;
			if (!a.rewind) {
				if (!e && c == r - 1)
					g = 1;
			...
			o(b[e], "ns-show");
			(a.c == "zoom" || a.c == "kb") && b[e].nI && i.a.insertRule
					&& ib(b[e].nI, e, b[e].dL)
		}
		Cb(e);
		var q = c;
